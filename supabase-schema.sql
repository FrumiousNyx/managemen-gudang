...
-- Create suppliers table (Supplier Kain - 3 fix: Bapa Cecep, Dian, Garmen)
...
-- Create vendors table (Vendor Jahit - 3-4 vendors)
CREATE TABLE IF NOT EXISTS vendors (
  ...
  email TEXT,
  address TEXT,
  production_days TEXT[], -- ['monday', 'tuesday', 'wednesday', 'thursday', 'friday']
  notes TEXT,
  is_active BOOLEAN DEFAULT true,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);

-- Create raw_materials table (Kain belum jadi)
CREATE TABLE IF NOT EXISTS raw_materials (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  supplier_id UUID NOT NULL REFERENCES suppliers(id) ON DELETE SET NULL,
  name TEXT NOT NULL,
  type TEXT NOT NULL, -- 'kain', 'benang', dll
  color TEXT,
  quantity INTEGER NOT NULL, -- jumlah rol
  unit TEXT NOT NULL DEFAULT 'rol',
  unit_price DECIMAL(10,2), -- harga per rol
  status TEXT DEFAULT 'available', -- 'available', 'in_production', 'used'
  notes TEXT,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE IF NOT EXISTS raw_material_logs (
  ...
  type TEXT NOT NULL CHECK (type IN ('INBOUND', 'OUTBOUND', 'TO_PRODUCTION')),
  qty REAL NOT NULL,
  notes TEXT,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);

-- Create production_orders table (Order ke Vendor)
CREATE TABLE IF NOT EXISTS production_orders (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  vendor_id UUID NOT NULL REFERENCES vendors(id) ON DELETE SET NULL,
  product_id UUID NOT NULL REFERENCES products(id) ON DELETE CASCADE,
  sku TEXT NOT NULL,
  rol_count INTEGER NOT NULL, -- jumlah rol kain
  output_qty INTEGER NOT NULL, -- MANUAL INPUT hasil pcs
...
CREATE TABLE IF NOT EXISTS production_logs (
  ...
  type TEXT NOT NULL CHECK (type IN ('ORDER_PLACED', 'IN_PROGRESS', 'COMPLETED', 'CANCELLED')),
  qty INTEGER,
  notes TEXT,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);

-- Create production_summary table (Tabel Produksi di Gudang)
CREATE TABLE IF NOT EXISTS production_summary (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  vendor_id UUID NOT NULL REFERENCES vendors(id) ON DELETE SET NULL,
  product_id UUID NOT NULL REFERENCES products(id) ON DELETE CASCADE,
  sku TEXT NOT NULL,
  rol_count INTEGER NOT NULL,
  output_qty INTEGER NOT NULL, -- hasil pcs manual input
...
-- Indexes for production system
CREATE INDEX IF NOT EXISTS idx_suppliers_name ON suppliers(name);
CREATE INDEX IF NOT EXISTS idx_suppliers_active ON suppliers(is_active);
CREATE INDEX IF NOT EXISTS idx_vendors_name ON vendors(name);
CREATE INDEX IF NOT EXISTS idx_vendors_active ON vendors(is_active);
CREATE INDEX IF NOT EXISTS idx_vendors_production_days ON vendors USING GIN(production_days);
...
CREATE INDEX IF NOT EXISTS idx_raw_material_logs_raw_material_id ON raw_material_logs(raw_material_id);
CREATE INDEX IF NOT EXISTS idx_production_orders_vendor_id ON production_orders(vendor_id);
...
CREATE INDEX IF NOT EXISTS idx_production_summary_vendor_id ON production_summary(vendor_id);
CREATE INDEX IF NOT EXISTS idx_production_summary_date ON production_summary(production_date);
CREATE INDEX IF NOT EXISTS idx_products_supplier_id ON products(supplier_id);

-- Insert sample suppliers
INSERT INTO suppliers (name, contact_person, phone, email, is_active) VALUES
  ('Bapa Cecep', 'Cecep', '081234567890', 'cecep@example.com', true),
  ('Dian', 'Dian', '081234567891', 'dian@example.com', true),
  ('Garmen', 'Garmen Admin', '081234567892', 'garmen@example.com', true)
ON CONFLICT DO NOTHING;

-- Insert sample vendors
INSERT INTO vendors (name, contact_person, phone, email, production_days, is_active) VALUES
  ('Iwan', 'Iwan', '081234567893', 'iwan@example.com', ARRAY['monday', 'tuesday'], true),
